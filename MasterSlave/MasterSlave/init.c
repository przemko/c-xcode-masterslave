...
void Init(int n)
{
    ...
    print_log(INITLOGID, "tworzę semafor");
    semaphore_id = semget(IPC_PRIVATE, 1,
                          IPC_CREAT | IPC_EXCL | S_IRUSR | S_IWUSR); // sprawdzić stałe!!!
    
    print_log(INITLOGID, "inicjuję semafor wartością 0 (zamknięty)");
	...
	print_log(INITLOGID, "piszę do współdzielonej pamięci polecenie WAIT");
	sprintf(shared_memory, "WAIT");
    ...
    print_log(INITLOGID, "otwieram semafor");
    binary_semaphore_post(semaphore_id); // UWAGA: jeśli zakomentować to procesy będą czekać!
